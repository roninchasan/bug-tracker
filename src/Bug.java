...
public class Bug implements Runnable {
	...
	private void step() {
		...
		for (Person p : people) {
			if (p.isSick()) {
				...
				if (n != null && n.isHealthy() &&
					statistics.bugSpreadsToPerson()) {
					n.becomeSick(steps);
					tree.insert(p, n);
				}
			}
	...
	private static Tuple5<Integer, Integer, Double, Double, Double> readArgs() {
		...
		int size= ScannerUtils.get(Integer.class, scanner,
		...
		double immunizationProb= ScannerUtils.get(Double.class, scanner,
			...
			"a double in the range [0,1]", (d) -> d >= 0 && d <= 1);
		scanner.close();
		return Tuple.of(size, maxHealth, connectionProb, sicknessProb,
			immunizationProb);
	}

	/** Run Bug on the arguments listed in args. <br>
	 * If args doesn't match the pattern below, read in arguments via the console by using readArgs().
	 *
	...
	public static void main(String[] args) {
		...
		System.out.print(".");
		Statistics s= new Statistics(sicknessProbability,
